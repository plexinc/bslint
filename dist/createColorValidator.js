...
exports.createColorValidator = void 0;
const diagnosticMessages_1 = require("./plugins/codeStyle/diagnosticMessages");
function createColorValidator(severity) {
    ...
    return (text, range, diagnostics) => {
        ...
        if ((colorFormat === 'never') && (quotedNumericHexMatches || hashHexMatches)) {
            diagnostics.push(diagnosticMessages_1.messages.expectedColorFormat(range));
            return;
        }
...
exports.createColorValidator = createColorValidator;
function validateColorAlpha(alphaMatches, hashMatches, quotedNumericHexMatches, range, diagnostics, alpha, alphaDefaults) {
    const validateColorAlpha = (alpha === 'never' || alpha === 'always' || alpha === 'allowed');
    if (validateColorAlpha) {
        if (alpha === 'never' && alphaMatches) {
            diagnostics.push(diagnosticMessages_1.messages.expectedColorAlpha(range));
        }
        ...
        if ((alphaDefaults === 'never' || alphaDefaults === 'only-hidden') && alphaMatches) {
            for (let i = 0; i < alphaMatches.length; i++) {
                ...
                if (alphaValue === 'ff' || (alphaDefaults === 'never' && alphaValue === '00')) {
                    diagnostics.push(diagnosticMessages_1.messages.expectedColorAlphaDefaults(range));
                }
            }
        }
    }
}
function validateColorCase(matches, range, diagnostics, colorCase, colorFormat) {
    ...
    if (validateColorCase && matches) {
        ...
        if (colorCase === 'lower' && colorValue !== colorValue.toLowerCase()) {
            diagnostics.push(diagnosticMessages_1.messages.expectedColorCase(range));
        }
...
function validateColorCertCompliance(matches, range, diagnostics, colorFormat, certCompliant) {
    ...
    if (validateCertCompliant && matches) {
        ...
        if (colorLuma > MAX_WHITE_LUMA || colorLuma < MAX_BLACK_LUMA) {
            diagnostics.push(diagnosticMessages_1.messages.colorCertCompliance(range));
        }
    }
}
function getColorLuma(value) {
...
//# sourceMappingURL=createColorValidator.js.map